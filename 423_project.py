...
previousTime = 0
elapsedTime = 0
cre_speed = 30  # Initial creature speed
level_speed_increment = 20  # Speed increment per level up
score = 0
lvl2 = 2
lvl3 = 10
status = 'playing'

# Global variables (add these in your existing global declarations)
special_active = False      # To track if the special circle effect is active
special_timer = 0           # Timer to track how long the immunity lasts
special_circle = None       # Store the special circle position for collision detection
...
def draw_creatures():
    ...
    global special_active, special_timer, special_circle

    if special_active and (time.time() - special_timer) >= 10:
        special_active = False  # Deactivate special effect after 10 seconds
...
def animation():
    ...
    if status == "gameover":
        ...
        glutPostRedisplay()  # Continue to render the Game Over message
        return
    if cre_cx != []:
        ...
        if status == "playing":
            # Adjust creature speed based on score and levels
            if score > lvl3:
        ...
        for i in range(len(cre_cx)):
            if cre_cx[i] >= -50 and cre_start[i] == 1050:  # Object moves leftward if it started at right end
                cre_cx[i] -= cre_speed * elapsedTime / 1e3
            elif cre_cx[i] <= 1050 and cre_start[i] == -50:  # Object moves rightward if it started at left end
                cre_cx[i] += cre_speed * elapsedTime / 1e3
